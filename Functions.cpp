...
// [[Rcpp::export]]
List VUS_ID_t(double t, int t_index, NumericVector ill_times, NumericVector death_times, List transition_probs) {
  ...
  ind_initial=ind[ill_times > t & death_times > t];
  ind_ill=ind[ill_times < t & death_times > t];
  ind_death=ind[death_times==t];
...
// [[Rcpp::export]]
List VUS_CD_t(double t, int t_index, NumericVector ill_times, NumericVector death_times, List transition_probs) {
...
List VUS_ID(NumericVector times,
  ...
  for(int i=0; i < n; i++) {
    /*Rcout << i << std::endl;*/
    List vus_t=VUS_ID_t(times(i),i,ill_times,death_times,transition_probs);
    result(i)=vus_t(0);
    conc(i)=vus_t(1);
    comp(i)=vus_t(2);
  }
...
List VUS_CD(NumericVector times,
  ...
  NumericVector comp(n);
  for(int i=0; i < n; i++) {
    List vus_t=VUS_CD_t(times(i),i,ill_times,death_times,transition_probs);
    result(i)=vus_t(0);
    conc(i)=vus_t(1);
    comp(i)=vus_t(2);
  }
